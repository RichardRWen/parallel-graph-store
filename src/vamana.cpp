...
struct weighted_vertex {
	uint64_t vertex_id; // TODO: change this id type
	double weight;
};
...
beam_search(Point p, Graph<indexType> &G, std::vector<Point> &points,
		parlay::sequence<indexType> starting_points, int beam_size, int alpha) {
	...
	// TODO: compare sorted list frontier with min-max heap frontier
	...
	while (remain > 0 && num_visited < QP.limit) {
		...
		// TODO: compare std::set_union to custom set_union that cuts off at max size
